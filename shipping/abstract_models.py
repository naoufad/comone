...
# Ajout pour la livraison par poids et region 
...
CheckoutSessionData = get_class('checkout.utils',
                                'CheckoutSessionData')
Repository = get_class('shipping.repository', 'Repository')
CheckoutSessionMixin=get_class('checkout.session',
...
### Transport avec Poids et Région 


#shipping_addr=CheckoutSessionMixin.get_shipping_address(self,basket)
"""   
class ChargeRegion(Repository,CheckoutSessionMixin):
 #   def get_shipping_address(self, basket):
  #      pass
    
   # shipping_addr=get_shipping_address
    
    def get_charge_region( self, basket, shipping_addr=None, **kwargs):
        if shipping_addr is not None:
            if ShippingAddress.MORONI==shipping_addr.line2:
                region=shipping_methods.Moroni.charge_incl_tax
        ...
        return region
"""
#region=ChargeRegion.get_charge_region
...
#    def get_charge_region( self, basket, shipping_addr=None, **kwargs):
   ...
   # shipping_addr=CheckoutSessionMixin.get_shipping_address(self,basket)
    ...
    def get_shipping_address(self, basket):
        ...
        shipping_addr = super(AbstractWeightBased, self).get_shipping_address( basket )
    ...
    def calculate(self, basket):
        ...
        charge+=region
        #ChargeRegion.get_charge_region(self,shipping_addr) 
